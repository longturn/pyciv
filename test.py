...
def read_sections(section_class, sections):
    ...
    for section in sections:
        if section_class._section_regex.match(section.name):
            ...
            fields = list(filter(lambda name: not name.startswith('_'),
                                 dir(section_class)))
            ...
            for name, value in section.items():
                if hasattr(section_class, '_rewrite_rules') and name in section_class._rewrite_rules:
                    name = section_class._rewrite_rules[name]

                if not name in fields and not name in annotations:
                    raise TypeError(f'Type {section_class.__name__} has no field called "{name}"')
...
def read_section(section_class, sections):
    ...
    if not all_results:
        raise ValueError(f'No section matching "{section_class._section_regex}" was found')
    if len(all_results) > 1:
        raise ValueError(f'Several sections matching "{section_class._section_regex}" were found')
...
def process_ruleset(path, ruleset):
    ...
    all_advances = {advance.name: advance for advance in read_sections(Advance, tech_sections)}
    ...
    def find_requirement(advance, name, all_advances):
        if name is None:
            ...
            raise ValueError('Advance "%s" references unknown advance "%s"' % (advance.name, name))
    ...
    all_unit_classes = {unit_class.name: unit_class for unit_class in read_sections(UnitClass, units_sections)}
    all_unit_types = {unit_type.name: unit_type for unit_type in read_sections(UnitType, units_sections)}
    ...
    for unit_type in all_unit_types.values():
        if unit_type.uclass in all_unit_classes:
            ...
            raise ValueError('Unit type "%s" references unknown unit class "%s"' % (unit_type.name, unit_type.uclass))
        ...
        if unit_type.obsolete_by in all_unit_types:
            ...
            raise ValueError('Unit type "%s" references unknown unit type "%s"' % (unit_type.name, unit_type.obsolete_by))
        ...
        for target in unit_type.targets:
            if target in all_unit_classes:
                ...
                raise ValueError('Unit type "%s" references unknown target "%s"' % (unit_type.name, target))
        ...
        for req in unit_type.tech_req:
            if req in all_advances:
                ...
                raise ValueError('Unit type "%s" references unknown advance "%s"' % (unit_type.name, req.name))
        ...
        if unit_type.cargo:
            for name in unit_type.cargo:
                if name in all_unit_classes:
                    ...
                    raise ValueError('Unit type "%s" references unknown unit class "%s"' % (unit_type.name, name))
    ...
    all_buildings = {building.name: building for building in read_sections(Building, buildings_sections)}
    ...
    for advance in all_advances.values():
        required_by = list(filter(lambda adv: advance in adv.reqs, all_advances.values()))
        hard_required_by = list(filter(lambda adv: advance == adv.root_req, all_advances.values()))
        required_by_units = list(filter(lambda ut: advance in ut.tech_req, all_unit_types.values()))
        with open('rules/%s/tech/%s.html' % (ruleset, make_slug(advance.name)), 'w') as out:
            out.write(template.render(
                advance=advance,
    ...
    for unit_class in all_unit_classes.values():
        units_in_class = filter(lambda ut: ut.uclass == unit_class, all_unit_types.values())
        with open('rules/%s/unit_class/%s.html' % (ruleset, make_slug(unit_class.name)), 'w') as out:
            out.write(template.render(unit_class=unit_class,
    ...
    for unit_type in all_unit_types.values():
        obsolete = list(filter(lambda ut: ut.obsolete_by == unit_type, all_unit_types.values()))
        with open('rules/%s/unit/%s.html' % (ruleset, make_slug(unit_type.name)), 'w') as out:
            out.write(template.render(
                unit_type=unit_type,
                obsolete=obsolete,
    ...
    for building in all_buildings.values():
        with open('rules/%s/building/%s.html' % (ruleset, make_slug(building.name)), 'w') as out:
            out.write(template.render(
                building=building,
                html_description=markdown(building.helptext)))
...
for ruleset in ('civ1', 'civ2', 'civ2civ3', 'classic', 'experimental',
                'multiplayer', 'sandbox'):
    process_ruleset(['/usr/share/freeciv'], ruleset)

for ruleset in (('AU1', 'augmented2'), 'augmented2', 'MP3_26', ('LT30', 'lt30'),
                'LT31', 'LT32', 'LT33', 'LT34', 'LT35', 'LT36', 'LT37', 'LT38',
                'LT39', 'LT40', 'LT41', #'LT42', # Effects are broken
                'LT43', 'LT44', 'LT45', 'LT46',
                'LT47', #'LT48', # Has a syntax error
                'LT49', 'LT50', 'LT51', 'LT52', 'LT53', 'LearningLT'):
    if not type(ruleset) is tuple:
        ruleset = (ruleset, ruleset)
    process_ruleset([f'/home/louis/freeciv/ltnet/games/{ruleset[0]}/data',
                     '/usr/share/freeciv'],
                    ruleset[1])
