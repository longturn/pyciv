...
def parse_files(ruleset, data_path):
    ...
    for name in (
        ...
        try:
            ...
            # in the path.
            _log.warn(
                "Could not find %s/%s.ruleset, trying default/%s.ruleset",
                ruleset,
                name,
                name,
            )
            _log.warn(
                "This behavior is supported for backwards compatibility. "
                "Use *include instead."
            )
...
def check_version(all_sections):
    ...
    for name, sections in all_sections.items():
        ...
        if version and version != header_version:
            raise ValueError(
                "Conflicting versions in ruleset files: "
...
def document_ruleset():
    ...
    parser = argparse.ArgumentParser(description="Create documentation for a ruleset")
    ...
    parser.add_argument(
        "-p", "--path", action="append", help="add a directory to the search path"
    )
