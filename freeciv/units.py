...
def load_veteran_levels(veteran_names,
    ...
    if any((len(veteran_names) != len(veteran_raise_chance),
    ...
    if levels[-1].raise_chance != 0 or levels[-1].work_raise_chance != 0:
        warn('The last veteran level "%s" has a non-zero raise chance' % levels[-1].name)
...
class UnitClass:
    ...
    def __post_init__(self):
        ...
        if unknown:
            raise ValueError('Unit class "%s" has unknown flags %s' % (self.name, unknown))
    ...
    def __post_init__(self):
        ...
        if self.veteran_names:
            self.veteran_levels = load_veteran_levels(
                self.veteran_names, self.veteran_raise_chance,
                self.veteran_work_raise_chance, self.veteran_power_fact,
