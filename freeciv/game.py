...
class CivStyleData:
    ...
    # Help Strings
    base_pollution_help_rst: str = "This value represents the base amount of pollution for each city. The value for this ruleset is: "

    happy_cost_help_rst: str = "This value represents the cost in luxury goods of making one citizen happier in a city. The value for this ruleset is: "

    food_cost_help_rst: str = "This value represents the amount of food it takes to maintain one citizen in a city. The value for this ruleset is: "

    granary_food_ini_help_rst: str = "This is a list of city granary sizes as cities grow from size 1 to ``x``, where ``x`` is the last value of the list. Most rulesets stop at size 8, so the list will have 8 values in it. Some rulesets have only one value, which means that the city granary does not increase with city size and remains stable. This ruleset has the following city granary sizes: "

    granary_food_inc_help_rst: str = "This value is related to the previous list above. If this value is greater than zero, it represents how much the city granary size grows for cities larger than the last value of the list above. This ruleset has the following city granary increment size value: "

    min_city_center_food_help_rst: str = (
        "The minimum amount of food a city center tile generates for this ruleset is: "
    )

    min_city_center_shield_help_rst: str = "The minimum amount of production (shields) a city center tile generates for this ruleset is: "

    min_city_center_trade_help_rst: str = (
        "The minimum amount of trade a city center tile generates for this ruleset is: "
    )

    init_city_radius_sq_help_rst: str = "This value represents the input to define the initial working area for a city. The math is somewhat complicated and is documented on the legacy freeciv wiki at https://freeciv.fandom.com/wiki/Radius. The larger the value, the larger the working area for the city. The initial city working area for this ruleset is: "

    init_vis_radius_sq_help_rst: str = "This value represents the input to define the initial vision area for a city. The math is somewhat complicated and is documented on the legacy freeciv wiki at https://freeciv.fandom.com/wiki/Radius. The larger the value, the larger the vision area for the city. The initial city vision area for this ruleset is: "
    ...
    autoupgrade_veteran_loss_help_rst: str = "Increments of ``10`` is one veteran level. The number of veteran levels lost when auto-upgrading a unit for this ruleset is: "

    pillage_select_loss_help_rst: str = "When set to ``True``, the player gets to select which terrain improvement to pillage. The value for this ruleset is: "

    tech_steal_allow_holes_help_rst: str = "When set to ``True``, the player can steal a technology advance for which prerequisites are not known. The value for this ruleset is: "

    tech_trade_allow_holes_help_rst: str = "When set to ``True``, the player can trade a technology advance to another player even if the other player has not discovered the prerequisite advance(s). This can happen in reverse as well. The value for this ruleset is: "

    tech_trade_loss_allow_holes_help_rst: str = "When set to ``True``, the player can lose a technology advance and not impact other prerequisite advances. The value for this ruleset is: "

    tech_parasite_allow_holes_help_rst: str = "When set to ``True``, the player can gain a technology advance for which prerequisites are not known via the ``parasite`` effect (e.g. Great Library). The value for this ruleset is: "

    tech_loss_allow_holes_help_rst: str = "When set to ``True``, the player can lose a technology advance with negative bulbs and not impact other prerequisite advances. The value for this ruleset is: "

    initial_diplomatic_state_help_rst: str = "This value represents the diplomatic state between two nations that meet for the first time. The value for this ruleset is: "

    civil_war_enabled_help_rst: str = (
        "When set to ``True``, civil war in enabled. The value for this ruleset is: "
    )

    civil_war_bonus_celebrating_help_rst: str = "This value indicates how many *percents* each celebrating city affects the chance of civil war. The value for this ruleset is: "
...
class IllnessData:
    ...
    # Help Strings
    illness_on_help_rst: str = "When set to ``True``, illness (plague) is enabled for the game. The value for this ruleset is: "

    illness_base_factor_help_rst: str = "This value defines the percentage factor of how plague is calculated. The value for this ruleset is: "
...
class CombatRulesData:
    ...
    tired_attack_help_rst: str = "When set to ``True``, units that attack with less than a single move point (per ``move_fragments`` in Terrain) will have their attack power reduced accordingly. The value for this ruleset is: "

    only_killing_makes_veteran_help_rst: str = "With some rules it is possible that neither side of a combat action dies. When this is set to ``True``, if unit should never gain veterancy from such a combat. The value for this ruleset is: "

    nuke_pop_loss_pct_help_rst: str = "The value defines the percentage of population lost by a city after a nuclear attak. If set to ``100``, the city is destroyed along with all the units. If set to ``0``, the city does not loose population. Any value below ``100`` means the city can never be destroyed completely using a nuclear unit. The value for this ruleset is: "

    nuke_defender_survival_chance_pct_help_rst: str = "The value defines the percentage chance of a city defender surviving a nuclear attack. When set to ``50``, roughly half of the defenders will survive a nuclear attack. When set to ``0`` no defenders will survive. When set to ``100`` all defenders will survive. The value for this ruleset is: "

    killstack_help_rst: str = "When set to ``True``, a stack of units can be completely destroyed when the strongest defender is killed. The value for this ruleset is: "
...
class AutoAttackData:
    ...
    # Help Strings
    attack_actions_help_rst: str = "An auto attack may be triggered when another unit moves to an adjacent tile and the ``autoattack`` server setting is enabled. The values are the possible actions. The value for this ruleset is: "
    if_attacker_help_rst: str = "This is a requirement vector table listing the rules that all must be true for auto attack actions to occur. The value for this ruleset is: "
    will_never_help_rst: str = "? help needs help ? The value for this ruleset is: "
...
class ActionsData:
    ...
    # Help Strings
    force_trade_route_help_rst: str = "If set to ``True``, it is illegal for a unit to enter the marketplace of a city if it can establish a trade route to it instead. The value for this ruleset is: "

    force_capture_units_help_rst: str = "If set to ``True``, it is illegal for a unit to bombard, explode a nuclear or perform a regular attack against a tile if it can capture units on it instead. The value for this ruleset is: "

    force_bombard_help_rst: str = "If set to ``True``, it is illegal for a unit to explode a nuclear or perform a regular attack against a tile if it can bombard it instead. The value for this ruleset is: "

    force_explode_nuclear_help_rst: str = "If set to ``True``, it is illegal for a unit to perform a regular attack against a tile if it can explode a nuclear unit instead. The value for this ruleset is: "

    poison_empties_food_stock_help_rst: str = "If set to ``True``, a successful `Poison City` or `Poison City Escape` action will empty the food stock (city granary). The value for this ruleset is: "
    ...
    bombard_2_max_range_help_rst: str = "The maximum distance from the unit to the target of the `Bombard 2` action. The value ``1`` means that the targets must be on a tile adjacent to the unit. The special value ``unlimited`` lifts the maximum distance restriction. The maximum distance cannot be smaller than the minimum distance. The value for this ruleset is: "

    bombard_3_max_range_help_rst: str = "The maximum distance from the unit to the target of the `Bombard 3` action. The value ``1`` means that the targets must be on a tile adjacent to the unit. The special value ``unlimited`` lifts the maximum distance restriction. The maximum distance cannot be smaller than the minimum distance. The value for this ruleset is: "

    explode_nuclear_max_range_help_rst: str = "The maximum distance from the unit to the target of the `Explode Nuclear` action. The value ``0`` means that the target tile must be the tile of the unit. The value ``1`` means that the tile must be a tile adjacent to the unit. The special value ``unlimited`` lifts the maximum distance restriction. The maximum distance cannot be smaller than the minimum distance. The value for this ruleset is: "

    nuke_city_max_range_help_rst: str = "The maximum distance from the unit to the target of the `Nuke City` action. The value ``1`` means that the tile must be a tile adjacent to the unit. The special value ``unlimited`` lifts the maximum distance restriction. The maximum distance cannot be smaller than the minimum distance. The value for this ruleset is: "

    nuke_units_max_range_help_rst: str = "The maximum distance from the unit to the target of the `Nuke Units` action. The value ``1`` means that the tile must be a tile adjacent to the unit. The special value ``unlimited`` lifts the maximum distance restriction. The maximum distance cannot be smaller than the minimum distance. The value for this ruleset is: "

    airlift_max_range_help_rst: str = "The maximum distance from the unit to the target of the `Airlift Unit` action. The value ``1`` means that the targets must be on a tile adjacent to the unit. The special value ``unlimited`` lifts the maximum distance restriction. The maximum distance cannot be smaller than the minimum distance. The value for this ruleset is: "
...
class BordersData:
    ...
    # Help Strings
    radius_sq_city_help_rst: str = "This value represents the input to define the initial border area outside a city. The math is somewhat complicated and is documented on the legacy freeciv wiki at https://freeciv.fandom.com/wiki/Radius. The larger the value, the larger the border area for the city. The value for this ruleset is: "
    size_effect_help_rst: str = "The border area increases by this amount divided by the of city size. The value for this ruleset is: "
    radius_sq_city_permanent_help_rst: str = "This value is the difference between a city's workable area and the area permanently claimed by the city. These tiles cannot be stolen by a stronger border source. A value of ``0`` means the city workable area is immune to border stealing. A negative value means outer workable tiles can be stolen. Highly negative values (more than max city area) means any workable tile can be stolen. If the city area value is variable, so is the set of locked tiles. This is a squared value, so the radius of the ring of tiles which are workable, but not locked (or vice versa) varies but the area is constant. The value for this ruleset is: "
...
class ResearchData:
    ...
    tech_leakage_help_rst: str = "This value defined how technology leakage occurs from other civilizations. `None`: There is no reduction of the technology cost. `Embassies`: The technology cost is reduced depending on the number of players which already know the technology and you have an embassy with. `All Players`: The technology cost is reduced depending on the number of all players (human, AI and barbarians) which already know the technology. `Normal Players`: The technology cost is reduced depending on the number of normal players (human and AI) which already know the technology. The value for this ruleset is: "

    tech_upkeep_style_help_rst: str = "This is the method of paying technology upkeep. `None`: This is no technology upkeep. `Basic`: The technology upkeep is calculated as: :math:`<Cost of Technology> / tech_upkeep_divider - tech_upkeep_free`. `Cities`: The technology upkeep is calculated like `Basic`, but multiplied by the number of cities. The value for this ruleset is: "
...
class CultureData:
    ...
    # Help Strings
    victory_min_points_help_rst: str = "The minimum culture points for cultural domination victory. The value for this ruleset is: "

    victory_lead_pct_help_rst: str = "How big of a lead relative to second best player that is needed for cultural domination victory. The value for this ruleset is: "

    migration_pml_help_rst: str = "How much existing history grows each turn. This makes older history of the same  original value more valuable as newer history, as it has gained more interest. The value for this ruleset is: "

    history_interest_pml_help_rst: str = "How much each culture point affects the migration from/to the city. Each culture point counts as this many permilles of a migration point. The value for this ruleset is: "
...
class CalendarData:
    ...
    # Help Strings
    start_year_help_rst: str = "This value defines the year the game starts. The value for this ruleset is: "

    skip_year_0_help_rst: str = "If set to ``True`` the game starts at Year 1. The value for this ruleset is: "

    fragments_help_rst: str = "The game calendar year can be broken into fragments. This value defines the number of year fragments. The value for this ruleset is: "

    fragment_name0_help_rst: str = "The first calendar year fragment name. The value for this ruleset is: "

    fragment_name1_help_rst: str = "The second calendar year fragment name. The value for this ruleset is: "

    fragment_name2_help_rst: str = "The third calendar year fragment name. The value for this ruleset is: "
